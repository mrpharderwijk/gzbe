...
export class TechService {
  ...
  findAll(sourceId: string = 'all'): Observable<Article[]> {
    ...
    if (!ObjectHelper.hasPath(api, `feeds.tech.sources`)) {
      return;
    }
     ...
     */
    const newsSource = api.feeds.tech.sources.find(
      (source: NewsSource) => source.id === sourceId,
    );

    if (!newsSource || sourceId === 'all') {
      observableStream = api.feeds.tech.sources.map((source: NewsSource) => {
        return this.retrieveArticles(api.feeds.tech, source.id);
      });
    }
     ...
     */
    if (newsSource && sourceId !== 'all') {
      observableStream = this.retrieveArticles(api.feeds.tech, newsSource.id);
    }
  ...
  findAllTechSources(): NewsSource[] {
    return this.configService.get('api').feeds.tech.sources;
  }
}
